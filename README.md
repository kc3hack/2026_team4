...
### アイデア面
* **写真がキャラになる「メイティ生成」**
  日常のモノを撮影するだけで、世界に一匹だけのオリジナルキャラクターを作成。現実世界とゲームがリンクする体験を作りました。

* ** 機械学習によるステータス判定**
  スキャン画像の類似度や特徴を解析し、キャラの「タイプ」や「ステータス」を自動付与するアルゴリズムを実装しました。

* ** 繋がりを生む「バトル＆交換システム」**
  生成したキャラ同士を戦わせたり交換したりする機能を搭載。「それ何撮ったの？」というリアルなコミュニケーションを誘発します。
### デザイン面
